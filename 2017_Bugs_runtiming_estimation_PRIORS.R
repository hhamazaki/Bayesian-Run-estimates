...
results_file = 'GEO_Chinook_bug_results.csv'
...
#    y[j,i] ~ dnorm(log.theta[j,i], tausqd[i])
...
#  a: is independent not hierarhical 
   a[i] ~ dnorm(a0,a0.prec)
#  a[i] ~ dnorm(0,0.00001)%_%I(0,)
  b[i] ~ dnorm(b0,b0.prec)%_%I(0.16,)
  mu[i] ~ dnorm(mu0,mu0.prec)%_%I(0,)   
     }  
		a0 ~ dnorm(4,0.001)%_%I(0,)
  	    b0 ~ dnorm(0.2,0.1)%_%I(0.16,)
		mu0 ~ dnorm(50,0.001)%_%I(10,)
	a0.prec <-1/a0.ssq 
    a0.ssq <- a0.sigma*a0.sigma
    a0.sigma ~ dunif(0,1)  	
	b0.prec <-1/b0.ssq 
...
parameters <- c('a','b','mu','a0','b0','mu0','a0.sigma','b0.sigma','mu0.sigma','y2','theta') 
...
am <- exp(bug_summary[substr(row.names(bug_summary),1,2)=='a[',5])
